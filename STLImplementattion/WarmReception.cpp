//Warm Reception
//Send Feedback
//There is only one beauty parlour in the town CodingNinjasLand. The receptionist at the beauty parlor is flooded with appointment requests because the “Hakori”
// festival is round the corner and everyone wants to look good on it.
...
//Input Format :
//First line contains the number of customers that will come. Second line contains N space-separated integers which 
//represent the arrival timings of the customer. Third line contains N space-separated integers which represent the departure timings of the customer. Arrival and departure timings are given in 24-hour clock.
//Constraints:
//1<= N <= 100
...
int i=0,j=0,chairs=0,count=0;
while(i<cust_number)
{
    if(entry[i]<exit[j])
    {count=count+1;
     i++;
     chairs=max(count,chairs);
    }
    else
    {
        count=count-1;
